...
;~ 	  MsgBox (0, "testpath", "\??\" & @ScriptDir &"\"& $arch &"\drivers\vboxdrv\VBoxDrv.sys")
;~ 	  MsgBox (0, "testreg", RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxDRV", "ImagePath"))
      ...
      If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxDRV", "DisplayName") <> "VirtualBox Service" Then
		If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxDRV", "ImagePath") == "\??\" & @ScriptDir &"\"& $arch &"\drivers\vboxdrv\VBoxDrv.sys" Then
			Local $DRV = 0
		Else
...
;~ 	  MsgBox (0, "testpath", "\??\" & @ScriptDir &"\"& $arch &"\drivers\USB\filter\VBoxUSBMon.sys")
      ...
      If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxUSBMon", "DisplayName") <> "VirtualBox USB Monitor Driver" Then
        If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxUSBMon", "ImagePath") == "\??\" & @ScriptDir &"\"& $arch &"\drivers\USB\filter\VBoxUSBMon.sys" Then
			Local $MON = 0
		Else
...
;~ 	  MsgBox (0, "fLAG", "ADP="& $ADP &"DRV="& $DRV &"MON="& $MON &"NET="& $NET &"USB="& $USB)
...
Func HybridMode()
	If ProcessExists ("VirtualBox.exe") OR ProcessExists ("VBoxManage.exe") Then
		MsgBox ($MB_ICONERROR, "Virtualbox portable", "Virtualbox has run.")
		Exit
	EndIf
	if @OSArch="X64" Then
		$append_arch="64"
